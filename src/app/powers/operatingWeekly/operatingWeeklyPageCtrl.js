...
(function () {
    ...
    angular.module('BlurAdmin.power.operatingWeekly')
        .controller('operatingWeeklyPageCtrl', operatingWeeklyPageCtrl)
        .controller('addOperatingWeeklyPageCtrl', addOperatingWeeklyPageCtrl)
        .controller('editOperatingWeeklyPageCtrl', editOperatingWeeklyPageCtrl)
        .controller('modalDelCtrl', modalDelCtrl)

    /** @ngInject */
    function operatingWeeklyPageCtrl($scope,
                                     OperatingWeeklyList,
                                     OperatingWeeklyDel,
                                     reportTypeList,
                                     $state,
        ...
        $scope.getData = function (tableState) {
            ...
            OperatingWeeklyList.query(params, function(resp) {
                if (resp.code !== 200) {
                    ToastUtils.openToast('warning', resp.message);
                }
                $scope.show.data = resp.data
                $scope.show.operatingWeeklyArr = resp;
                ...
                $scope.show.operatingWeeklyArr.tableState = tableState;
            }, function(err) {
                console.log(err)
            })
        }
        ...
        $scope.refreshTable = function () {
            if (parseInt($scope.show.operatingWeeklyArr.totalPage) <= 1 && $scope.show.operatingWeeklyArr.tableState) {
                $scope.getData($scope.show.operatingWeeklyArr.tableState);
            } else {
                angular
        ...
        $scope.addReport = function () {
            ModalUtils.open('app/powers/operatingWeekly/widgets/createOperatingWeeklyModal.html', 'md',
                addOperatingWeeklyPageCtrl, {},
                function (info) {
                    // 传值走这里
                    if (info) {
                        $scope.clearForm();
        ...
        $scope.editReport = function (id) {
            ModalUtils.open('app/powers/operatingWeekly/widgets/editOperatingWeeklyModal.html',
                'md',
                editOperatingWeeklyPageCtrl, {id: id},
                function (info) {
                    // 传值走这里
                    if (info) {
                    }
    ...
    function addOperatingWeeklyPageCtrl ($scope,
                                         ToastUtils,
                                         OperatingWeeklyAdd,
                                         HttpToast,
                                         userCache) {
        ...
        $scope.add = function (data) {
            ...
            var reg = /\n|\r\n/g
            data.lastWeekSummary.replace(reg, "<br>");
            data.currentWeekPlan.replace(reg, "<br>");
            data.record.replace(reg, "<br>");
            data.summary.replace(reg, "<br>");
            OperatingWeeklyAdd.create(data, function(resp) {
                if (resp.code == 200) {
                    ToastUtils.openToast('success', resp.message);
                } else {
                    ToastUtils.openToast('warning', resp.message);
    ...
    function editOperatingWeeklyPageCtrl ($scope,
                                          OperatingWeeklyAttr,
                                          params) {
        OperatingWeeklyAttr.query({
            id: params.id
        }, function(resp) {
            console.log(resp)
            if (resp.code === 200) {
                var reg=new RegExp("<br>","g"); //创建正则RegExp对象
                resp.data.summary.replace(reg, '\n')
                resp.data.lastWeekSummary.replace(reg, '\n')
                resp.data.currentWeekPlan.replace(reg, '\n')
                resp.data.record.replace(reg, '\n')
